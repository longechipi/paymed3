...
if(isset($_FILES["datosexcel"])){
	...
	if($up->uploaded){
		...
		if($up->processed){
            ...
            $lei=0;$reg=0;$rec=0;$esta=0;$lnerr=1;$lineaserr='';
            for ($rowx = 2; $rowx <= $highestRow; $rowx++){
                $lei++;$lnerr++;
                $cedula = $sheet->getCell("A".$rowx)->getValue();
                $rif = $sheet->getCell("B".$rowx)->getValue();
                $apellido1 = strtoupper($sheet->getCell("C".$rowx)->getValue());
                $apellido2 = strtoupper($sheet->getCell("D".$rowx)->getValue());
                $nombre1 = strtoupper($sheet->getCell("E".$rowx)->getValue());
                $nombre2 = strtoupper($sheet->getCell("F".$rowx)->getValue());
                $correo = $sheet->getCell("G".$rowx)->getValue();
                $movil = $sheet->getCell("H".$rowx)->getValue();
                ...
                if ($cedula!='' && $rif!='' && $apellido1!='' && $nombre1!='' && $correo!='' && $movil!='') {
                    ...
                    $str1="SELECT count(*) as hay FROM medicos WHERE nrodoc='".$cedula."';";
                    $objlast1=$mysqli->query($str1); $arrlast1=$objlast1->fetch_array();  
                    $estacedula=$arrlast1[0];

                    $str2="SELECT count(*) as hay FROM medicos WHERE correo='".$correo."';";
                    $objlast2=$mysqli->query($str2); $arrlast2=$objlast2->fetch_array();  
                    $estacorreo=$arrlast2[0];

                    $str3="SELECT count(*) as hay FROM medicos WHERE rif='".$rif."';";
                    $objlast3=$mysqli->query($str3); $arrlast3=$objlast3->fetch_array();  
                    $estarif=$arrlast3[0];
                    ...
                    if ($estacedula=='0' && $estacorreo=='0' && $estarif=='0') { 
                      ...
                      $sqlbco = "INSERT INTO regpagos(idpagos, idmed, formapago, conptopago, bnco, nroref, monto, fechabco, fecinicio, fecfinal, observacion, idestatus) ";
                      $sqlbco .= " value (\"null\",\"$idmed\",\"$formapago\",\"$conptopago\",\"$bnco\",\"$nroref\",\"$monto\",\"$fecinicio\",\"$fecinicio\",\"$fecfinal\",\"$observacion\",\"1\")";
                      $con->query($sqlbco);
                      
                      //Mandar Mail a cada medico de la lista de excel
                      sleep(5);
                      include "sendmaillote.php";
                    }else {
                      //Datos ya existen
                      $lineaserr.='-'.$lnerr;
                    }
                ...
                <div class="col-md-12" align="center">
                  ...
                  <table class="tftable" border="1">
                      ...
                      <tr>
                       ...
                       <tr>
                        <th><h1>Datos Registrados<br>Lineas</h1></th>
