<?php
  ...
  $sqlbuscprivi = ("SELECT privilegios FROM loginn WHERE idlogin='".$idlogin."'; ");
  //echo $sqlbuscprivi; exit();
  $objbuscprivi = $mysqli->query($sqlbuscprivi);
  $rowbuscprivi = mysqli_fetch_array($objbuscprivi);
  $privilegios  = $rowbuscprivi['privilegios'];
...
if (isset($_POST['submit'])) {
    ...
    if($where==''){
      $sql = ("SELECT a.idprov, a.idlogin, a.rif, a.razsocial, a.dencomercial, a.idestatus, b.estado 
            FROM proveedores a, estado b WHERE a.idestado=b.idestado ORDER BY a.razsocial ASC");
    }else{
      $sql = ("SELECT a.idprov, a.idlogin, a.rif, a.razsocial, a.dencomercial, a.idestatus, b.estado 
            FROM proveedores a, estado b $where");
    }
...
<!-- -->
  ...
  <section class="content">
    ...
    <!-- //*FILTRO -->
          ...
          <div align="center">
               <button class="buttonr"><a style="color: #FFFFFF;font-family: 'Lato', sans-serif;font-size: 18px;font-weight: bold;" href="regprov.php">Añadir</a></button>
          </div>
          <br>
      <!-- Default box -->
      <div class="card">
        ...
        <div class="card-body p-0">
          <table class="table table-striped projects">
              ...
              <tbody>
				      <?php while($row = mysqli_fetch_array($result)) {$ln++; 
              ?>
              <tr>
                <td><?php echo $ln; ?></td>
                <td><a><?php echo $row['rif']; ?></a></td>
                <td class="project_progress"><?php echo substr($row['razsocial'],0,25);?>...</td>
                <td class="project_progress"><?php echo $row['dencomercial']; ?></td>
                <td class="project-state"><?php echo $row['estado']; ?></td>
                <td class="project-actions text-right">
                  <!--a class="btn btn-secondary btn-sm" href="aprobprov.php?id=< ?php echo $row['idprov'];?>" title="Aprobar Proveedor"><i class="fa fa-check"></i></a-->
                <?php if ($privilegios=='1' ) { //Admin ?>
                      <?php if ($row['idestatus']=='3'){ ?>
                          <button type="button" id="btnaprobar<?php echo $row['idprov'];?>" class="btn btn-success btn-sm" onclick="faproprov(<?php echo $row['idprov'];?>)" title="Aprobar Proveedor" ><i class="fa fa-check"></i></button>
                      <?php }else if ($row['idestatus']=='1' ){ ?>
                          <button type="button" id="btnaprobar<?php echo $row['idprov'];?>" class="btn btn-success btn-sm">OK</button>
                      <?php } ?>
                    <a class="btn btn-primary btn-sm" href="updprov.php?id=<?php echo $row['idprov'];?>"><i class="fa fa-edit"></i></a>
                    <a class="btn btn-danger btn-sm" href="src_del_prov.php?id=<?php echo $row['idprov'];?>"><i class="fa fa-trash"></i></a>
                <?php } else if ($privilegios=='2' ) { // Dr?>
                    <?php if ($row['idestatus']=='3'){ ?>
                          <button type="button" id="btnaprobar<?php echo $row['idprov'];?>" class="btn btn-warning btn-sm" title="Por Aprobar"><i class="fa fa-exclamation"></i></button>
                      <?php }else if ($row['idestatus']=='1' ){ ?>
                          <button type="button" id="btnaprobar<?php echo $row['idprov'];?>" class="btn btn-success btn-sm" title="Aprobado">OK</button>
                      <?php } ?>
