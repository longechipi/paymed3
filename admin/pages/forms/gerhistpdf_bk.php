<?php
  ...
  date_default_timezone_set('America/Caracas');
    ...
    $sqlhist = ("SELECT a.idpaci, a.idcita, b.idmed, concat(b.nombre1,' ', b.apellido1) as nombremedico, b.rif, b.codcolemed, b.mpss, b.correo, concat(b.codarea,'-', b.telefono) as telefonolocal, a.*, concat(c.nombre1,' ', c.apellido1) as nombrepaciente, c.cedula, c.edad, a.fechadia as fechaconsulta, a.hora as horaconsulta
    FROM consultas_med a, medicos b, pacientes c
    WHERE a.idmed=b.idmed and a.idpaci=c.idpaci and a.idregistro='".$idrg."';");
    
    $arrhist=$mysqli->query($sqlhist); 	$rowhist = mysqli_fetch_array($arrhist);
    // Armo -nombre del PDF
    $nombrepdf=$rowhist['nombrepaciente']."_".substr($rowhist['fechaconsulta'],0,2)."_".substr($rowhist['fechaconsulta'],3,2)."_".substr($rowhist['fechaconsulta'],6,4).".pdf";
...
tr, th, td {
   padding:2px
        ...
        $html.='</header>
        <footer>'.$nombrecentrosalud.',  '.$calleav.' '.$casaedif .'<br>'.$estado.'-'.$pais.'
        . Telf.: (58.212) 508-60-07 / 574-6419 / Correo:'.strtolower($rowhist["correo"]).'<br>
        HORARIO DE ATENCION: ';
            while ($rowdia = mysqli_fetch_array($objdiaatiende)) {
              $horadesde=substr($rowdia["desde"],0,6);$horahasta=substr($rowdia["hasta"],0,6);
              $html.='-'.$rowdia["dia"];
            }
            $html.='<br>Desde:'.$horadesde;
            $html.='--'.$horahasta;
          ...
          <section class="content">
              ...
              <div class="container-fluid">
                ...
                <table style="width:100%" >
                      <tbody>
                          <tr> 
                              ...
                              <td style="text-align: right"><span style="font-size:17px;font-weight: bold">Fecha:'.'  '.$rowhist["fechadia"].'<br>
                                  Hora:</span>'.'  '.$rowhist["horaconsulta"].'
                              </td>
                          </tr>
                      </tbody>
                </table><hr>
                ...
                <table style="width:100%" >
                      <tbody>
                          <tr>
                              <td colspan="3" style="text-align: center"> <p style="font-family:italic;font-size:21px;">INFORME MEDICO</p></td>
                          </tr>
                          
                          <tr> 
                              <td style="text-align: center"><span style="font-weight: bold;">Peso:</span>'.$rowhist["peso"].'</td>
                              <td style="text-align: center"><span style="font-weight: bold;">Presion Arterial:</span>'.$rowhist["presion"].'</td>
                              <td style="text-align: center"><span style="font-weight: bold;">Fuma:</span>'.$rowhist["fumador"].'</td>
                          </tr>
                      </tbody>
                </table>
                <div>
                ...
                <table style="width:100%" cellpadding="0" >
                      <tbody>
                          <tr>
                            <td style="text-align: left"><span style="font-weight: bold;">Motivo:</span>'.$rowhist["motivo"].'</td>
                          </tr>
                          <tr>
                            <td style="text-align: left"><span style="font-weight: bold;">Antecedentes:</span>'.$rowhist["antecedentes"].'</td>
                            
                          </tr>
                          <tr>
                            <td style="text-align: left"><span style="font-weight: bold;">Examen Físico:</span>'.$rowhist["exfisico"].'</td>
                          </tr>
                          <tr>
                            <td style="text-align: left"><span style="font-weight: bold;">Hallazgos y Diagnóstico:</span>'.$rowhist["hallazgos"].'</td>
                          ...
                          if(!empty($rowhist['imagenologia'])){ 
                            ...
                            $html.=$rowhist["imagenologia"].'</td>
                                  </tr>';
                          }

                          if(!empty($rowhist['anatomia'])){ 
                          $html.='<tr>
                                    <td style="text-align: left"><span style="font-weight: bold;">Anatomia Patologica:</span>'.$rowhist["anatomia"].'</td>
                                  </tr>';
                          } 

                          if(!empty($rowhist['interconsultas'])){
                          $html.='<tr>
                                    <td style="text-align: left"><span style="font-weight: bold;">Interconsultas:</span>'.$rowhist["interconsultas"].'</td>
                                  </tr>';
                          } 

                          if(!empty($rowhist['otros'])){
                          $html.='<tr>
                                    <td style="text-align: left"><span style="font-weight: bold;">Otros Examenes:</span>'.$rowhist["otros"].'</td>
                                  </tr>';
                          }
                          $html.='<tr>
                                    <td>
