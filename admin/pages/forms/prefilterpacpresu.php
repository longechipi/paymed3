...
if (isset($_POST['submit'])) {
  ...
  $sqlpaci = ("SELECT count(cedula) as numero, cedula FROM pacientes WHERE cedula='".$ci."'");
    $arrpaci=$mysqli->query($sqlpaci); $rowpaci = mysqli_fetch_array($arrpaci);
      $cuan =$rowpaci['numero']; $pac =$rowpaci['cedula'];

  if($cuan>0){
      echo '<script language="javascript">
              window.location.href="pac_presu.php?idpac='.$pac.'"; </script>';
  }else{
      echo '<script language="javascript">
              alert("¡El paciente no posee consultas previas!");
              window.location.href="prefilterpacpresu.php"; </script>';
  }
